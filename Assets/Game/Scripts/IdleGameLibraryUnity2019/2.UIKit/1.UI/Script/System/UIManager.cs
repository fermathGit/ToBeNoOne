...
namespace QFramework {
    ...
    public class UIManager : QMgrBehaviour, ISingleton {
        ...
        public IPanel OpenUI(string uiBehaviourName, UILevel canvasLevel, IUIData uiData = null,
            string assetBundleName = null, bool isShowTween = false) {
            IPanel retPanel = null;

            uiBehaviourName = uiBehaviourName.LastWord();

            if (!mAllUI.TryGetValue(uiBehaviourName, out retPanel)) {
                retPanel = CreateUI(uiBehaviourName, canvasLevel, uiData, assetBundleName, isShowTween);
            }
        ...
        public void CloseAllUI() {
            foreach ( var layer in mAllUI ) {
                if ( layer.Key != "UIMainPanel"
                    && layer.Key != "UITopSubPanel"
                    && layer.Key != "UIGameGuidePanel"
                    && layer.Key != "UIHudPanel"
                    && layer.Key != "UILoadingPanel"
                    && layer.Key != "UITipsPanel"
                    && layer.Key != "UILevelUpPanel" ) {
                    HideUI( layer.Key );
                }
            }
        }
        ...
        public IPanel CreateUI(string uiBehaviourName, UILevel level = UILevel.Common, IUIData uiData = null,
            ...
            ui = UIPanel.Load("UI/Prefab/" + uiBehaviourName, assetBundleName);
            ...
            ui.PanelInfo = new UIPanelInfo { AssetBundleName = assetBundleName, Level = level, PanelName = uiBehaviourName };

            mAllUI.Add(uiBehaviourName, ui);

            ui.Init(uiData, isShowTween);
        ...
        public T OpenUI<T>(UILevel canvasLevel = UILevel.Common, IUIData uiData = null, string assetBundleName = null,
            string prefabName = null, bool isShowTween = false) where T : UIPanel {
            return OpenUI(prefabName ?? GetUIBehaviourName<T>(), canvasLevel, uiData, assetBundleName, isShowTween) as T;
        }
    ...
    public static class UIMgr {
        ...
        internal static T OpenPanel<T>(UILevel canvasLevel = UILevel.Common, IUIData uiData = null,
            ...
            string prefabName = null, bool isShowTween = false) where T : UIPanel {
            return UIManager.Instance.OpenUI<T>(canvasLevel, uiData, assetBundleName, prefabName, isShowTween);
        }

        internal static T OpenPanel<T>(IUIData uiData, string assetBundleName = null,
            string prefabName = null, bool isShowTween = false) where T : UIPanel {
            return UIManager.Instance.OpenUI<T>(UILevel.Common, uiData, assetBundleName, prefabName, isShowTween);
        }
